...
namespace AspectAbilities
{
    ...
    public class AspectAbilitiesPlugin : BaseUnityPlugin
    {
        ...
        {
            if (aspectAbility.equipmentDef == null)
            {
                logger.LogError("EquipmentDef missing in " + aspectAbility + " AspectAbility!");
                return;
            }
            RegisterAspectAbility(aspectAbility.equipmentDef, aspectAbility);
        }
    }
