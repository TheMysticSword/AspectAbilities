...
namespace TheMysticSword.AspectAbilities
{
    public class GenericGameEvents
    {
        ...
        public static event System.Func<DamageInfo, GenericCharacterInfo, GenericCharacterInfo, float, float> OnApplyDamageReductionModifiers;
        ...
        {
            Debug.LogError("generic game event '" + name + "' hook failed");
        }
            ...
            IL.RoR2.HealthComponent.TakeDamage += (il) =>
            {
                ...
                ))
                {
                    ...
                    c.EmitDelegate<System.Func<HealthComponent, DamageInfo, CharacterBody, float, float>>((healthComponent, damageInfo, attackerBody, damage) =>
                    {
                        ...
                        GenericCharacterInfo attackerInfo = new GenericCharacterInfo(attackerBody);
                        GenericCharacterInfo victimInfo = new GenericCharacterInfo(victimBody);
                        if (OnApplyDamageModifiers != null) damage = OnApplyDamageModifiers(damageInfo, attackerInfo, victimInfo, damage);
                        return damage;
                    });
                ...
                ))
                {
                    ...
                    c.EmitDelegate<System.Func<HealthComponent, DamageInfo, CharacterBody, float, float>>((healthComponent, damageInfo, attackerBody, damage) =>
                    {
                        ...
                        GenericCharacterInfo attackerInfo = new GenericCharacterInfo(attackerBody);
                        GenericCharacterInfo victimInfo = new GenericCharacterInfo(victimBody);
                        if (OnApplyDamageReductionModifiers != null) damage = OnApplyDamageReductionModifiers(damageInfo, attackerInfo, victimInfo, damage);
                        return damage;
                    });
                    c.Emit(OpCodes.Stloc, damagePosition);
                }
